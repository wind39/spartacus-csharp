...
(B) Patent Grant- Subject to the terms of this license, including the license conditions and 
 ...
 under its licensed patents to make, have made, use, sell, offer for sale, import, and/or otherwise dispose 
...
namespace Spartacus.ThirdParty
{
    ...
    public class ZipStorer : System.IDisposable
    {
        ...
        public OnProgress DoProgress = null;
        ...
        public void AddStream(Compression _method, string _filenameInZip, Stream _source, DateTime _modTime, string _comment)
        {
            ...
            long offset;
            if (this.Files.Count == 0) offset = 0; else { ZipFileEntry last = this.Files[this.Files.Count - 1]; offset = last.HeaderOffset + last.HeaderSize; }
        ...
        public string ExtractFile(ZipFileEntry _zfe, string _filename)
        {
            ...
            if (Directory.Exists(_filename)) return "";
            ...
            if (result) return _filename; else return "";
        }
        /// <summary>
        ...
        public bool ExtractFile(ZipFileEntry _zfe, Stream _stream)
        {
            ...
            uint bytesDone = 0;
            uint iter = 0;
            if (DoProgress != null) iter = DoProgress(_zfe, 0, bytesDone);
            while (bytesPending > 0)
            {
                int bytesRead = inStream.Read(buffer, 0, (int)Math.Min(bytesPending, buffer.Length));
                _stream.Write(buffer, 0, bytesRead);
                bytesPending -= (uint)bytesRead;
                bytesDone += (uint)bytesRead;
                if (DoProgress != null) iter = DoProgress(_zfe, iter, bytesDone);
            }
            _stream.Flush();
        ...
        */
        ...
        private void WriteLocalHeaderNoRef(ZipFileEntry _zfe)
        {
            long pos = this.ZipFileStream.Position;
            Encoding encoder = _zfe.EncodeUTF8 ? Encoding.UTF8 : DefaultEncoding;
            byte[] encodedFilename = encoder.GetBytes(_zfe.FilenameInZip);
        ...
        */
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            long sourceStart = 0;
            if (_source != null) if (_source.CanSeek) sourceStart = _source.Position;

            if (_zfe.Method == Compression.Store) outStream = this.ZipFileStream; else outStream = new DeflateStream(this.ZipFileStream, CompressionMode.Compress, true);
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    for (uint i = 0; i < bytesRead; i++) _zfe.Crc32 = ZipStorer.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    outStream.Flush();
                }
            } while (bytesRead > 0);
            //outStream.Flush();
        ...
        */
        ...
        public void UpdateCrcAndSizesNoRef(ZipFileEntry _zfe)
        {
            long lastPos = this.ZipFileStream.Position;  // remember position
